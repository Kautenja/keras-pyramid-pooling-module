...
def _hash(mapping: dict) -> str:
    ...
    # convert the string to bytes and return the MD5 hash of the bytes
...
def _rgb_to_onehot(metadata: pd.DataFrame) -> 'Callable':
    """
    ...
    def rgb_to_onehot(img_file: str, output_dtype: np.dtype) -> np.ndarray:
        ...
        """
        # load the data as a NumPy array and ignore any alpha channel, i.e.,
        # only take the RGB channels of the image
        img = imread(img_file)[..., :3]
...
def _class_and_file_totals(subset: str) -> pd.DataFrame:
    ...
    files = glob.glob(os.path.join(subset, '*.npz'))
...
def create_segmented_y(
    ...
    rgb_to_onehot = _rgb_to_onehot(metadata)

    # iterate over all the files in the source directory
    for img_file in tqdm.tqdm(sorted(glob.glob(y_glob)), unit='image'):
